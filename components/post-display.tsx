            ...
            {posts.map((post) => (
                <Suspense fallback={<Loading />} key={post.postid}>
                    <div className="postView">
                        {post.creator === currentUser ? <DeleteButton postId={post.postid}/> : null} 
                        <Link href={`/posts/${post.postid}`} className="grow">
                            ...
                            <p className="lg:text-3xl h-15 overflow-y-clip">{post.post}</p>
